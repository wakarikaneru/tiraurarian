class SearchController < ApplicationController
  ...
  def index
    ...
    if search_str.blank?
      ...
      @users = User.ransack(name_or_description_cont_any: query_array, id_does_not_match_all: my_mutes_array).result.order(last_tweet: :desc).limit(100)
      ...
      @tweets_tweet = Tweet.ransack(content_cont_any: query_array, id_does_not_match_all: my_mutes_array).result.order(id: :desc).limit(100)
    end
